...
template<class T, int m, int n>
T dot(const mat<T, m, n> &u, const mat<T, m, n> &v);
...
struct mat {
    ...
    void make_zero() { for (int i = 0; i < m; i++) for (int j = 0; j < n; j++) x[i][j] = 0; }
    ...
    mat &operator+=(const mat &v) {
        for (int i = 0; i < m; i++) for (int j = 0; j < n; j++) x[i][j] += v.x[i][j];
        return *this;
    }
    ...
    mat &operator*=(const T &c) {
        for (int i = 0; i < m; i++) for (int j = 0; j < n; j++) x[i][j] *= c;
        return *this;
    }
    ...
    mat operator-() const {
        mat r;
        for (int i = 0; i < m; i++) for (int j = 0; j < n; j++) r[i] = -x[i][j];
        return r;
    }

    mat operator+(const mat &v) const {
        mat r;
        for (int i = 0; i < m; i++) for (int j = 0; j < n; j++) r[i] = x[i][j] + v.x[i][j];
        return r;
    }
    ...
    mat operator*(const T &c) const {
        mat r;
        for (int i = 0; i < m; i++) for (int j = 0; j < n; j++) r[i] = x[i][j] * c;
        return r;
    }
...
template<class T, int m, int n>
mat<T, m, n> operator*(const T &c, const mat<T, m, n> &v) { return v * c; }
